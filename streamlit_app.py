...
# Prefill categories/subcategories (edit as needed)
...
def admin_login_ui():
    ...
    with st.expander("🔐 Admin login", expanded=False):
        ...
        if st.button("Sign in", type="primary", key="adm_signin_btn"):
...
def member_is_approved(email: str) -> bool:
    if not email:
        return False
    df = read_df("Members")
    if df.empty or "Email" not in df or "Approved" not in df:
        return False
...
# ===================== ADMIN ACTION HELPERS (no one-line if) =====================
...
with tabs[2]:
    ...
    with st.expander("Member quick sign-in (email for submissions & rating)"):
        ...
        if st.button("Set as me", key="me_set_btn"):
    ...
    if df.empty:
        ...
        c = st.columns(5)
        with c[0]:
            f_phase = st.selectbox("Phase", ["All"]+sorted(df["Phase"].dropna().unique().tolist()), key="f_phase")
        with c[1]:
            f_cat   = st.selectbox("Category", ["All"]+sorted(df["Category"].dropna().unique().tolist()), key="f_cat")
        with c[2]:
            f_srv   = st.selectbox("Service Type", ["All"]+sorted(df["Service_Type"].dropna().unique().tolist()), key="f_srv")
        with c[3]:
            f_wing  = st.selectbox("Wing", ["All"]+sorted(df["Wing"].dropna().unique().tolist()), key="f_wing")
        with c[4]:
            q       = st.text_input("Search", key="f_query")
        ...
        if "me" not in st.session_state:
            ...
            if not view.empty:
                ...
                choice = st.selectbox("Select listing", choices, key="rate_choice") if choices else None
                stars  = st.slider("Stars", 1, 5, 5, key="rate_stars")
                comment= st.text_input("Short comment (optional)", key="rate_comment")
                if st.button("Submit rating", key="rate_submit_btn"):
                    if choice:
    ...
    if "me" not in st.session_state:
        ...
        with st.form("dir_submit_form"):
            ...
            b_name = st.text_input("Business Name *", key="dir_name")
            ...
            with i1: u1 = st.text_input("Image URL 1", key="dir_i1")
            with i2: u2 = st.text_input("Image URL 2", key="dir_i2")
            with i3: u3 = st.text_input("Image URL 3", key="dir_i3")
...
with tabs[3]:
    ...
    if "me" not in st.session_state:
        ...
        with st.form("ven_submit_form"):
            ...
            i1,i2,i3 = st.columns(3)
            with i1: vu1 = st.text_input("Image URL 1", key="ven_i1")
            with i2: vu2 = st.text_input("Image URL 2", key="ven_i2")
            with i3: vu3 = st.text_input("Image URL 3", key="ven_i3")
...
with tabs[4]:
    ...
    with st.form("supp_form"):
        em = st.text_input("Your Email", key="supp_email")
        sub = st.text_input("Subject", key="supp_subject")
        msg = st.text_area("Message", height=120, key="supp_msg")
...
with tabs[6]:
    ...
    if not is_admin():
        ...
        with st.expander("Add Showcase (image/video)"):
            t = st.text_input("Title", key="ad_show_title")
            typ = st.selectbox("Type", ["image","video"], key="ad_show_type")
            url = st.text_input("URL (image link or video link)", key="ad_show_url")
            by  = st.text_input("Posted by", key="ad_show_by")
            notes = st.text_area("Notes", key="ad_show_notes")
            approve_now = st.checkbox("Approve now?", value=True, key="ad_show_approve")
            if st.button("Add to Showcase", type="primary", key="ad_show_btn"):
